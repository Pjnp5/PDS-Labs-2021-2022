...
public class JGalo extends JFrame implements ActionListener {
	...
	public static void main(String args[]) {
		...
		if (args.length == 0){
			...
			System.err.println("ERROR: Invalid input. You must select either 'X' or 'O'.");
			System.exit(1);
		}

		if (args[0].length() == 1){
			if ((Character.toUpperCase(args[0].charAt(0)) == 'X'  ) || (Character.toUpperCase(args[0].charAt(0)) == 'O')){
				new JGalo(new JGaloLogic(Character.toUpperCase(args[0].charAt(0))));
			} else {
				System.err.println("ERROR: Invalid input. You must select either 'X' or 'O'.");
				System.exit(1);
			}
